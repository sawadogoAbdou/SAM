...
public final class VisioTextExtractor extends POIOLE2TextExtractor {
	...
	private void findText(Stream stream, ArrayList<String> text) {
		...
		if(stream instanceof ChunkStream) {
			...
			for(int i=0; i<cs.getChunks().length; i++) {
				org.apache.poi.hdgf.chunks.Chunk chunk = cs.getChunks()[i];
				
				//System.out.print("Chunk : "+chunk.getName());
				if(chunk != null &&
						chunk.getName() != null &&
					...
					// First command
					Command cmd = (Command) chunk.getCommands()[0];
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.print("Data: "+extractor.getText());
